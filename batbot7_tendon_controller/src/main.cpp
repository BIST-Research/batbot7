...
void loop(){
  if (Serial.available())
  {
    ...
    tendon_datapacket_t tendon_datapacket_ptr;
    unpack(buff, &tendon_datapacket_ptr);

    char outbuff[100];

    switch (tendon_datapacket_ptr.opcode)
    {
      case READ_STATUS:
        sprintf(outbuff, "Got read status opcode");
        break;
      case READ_ANGLE:
        sprintf(outbuff, "Got read angle opcode");
        break;
      case WRITE_ANGLE:
        sprintf(outbuff, "Got write angle opcode");
        break;
      case WRITE_PID:
        sprintf(outbuff, "Got write pid opcode");
        break;
      default:
    ...
    // sprintf(outbuff, "Received packet: with %x params, motorId: %x, opcode: %x",
    //   tendon_datapacket_ptr.numParams,
    //   tendon_datapacket_ptr.motorId,
    //   tendon_datapacket_ptr.opcode
    // );

    Serial.write(outbuff);
    Serial.flush();

    // try to read in an entire data packet 
  }
}
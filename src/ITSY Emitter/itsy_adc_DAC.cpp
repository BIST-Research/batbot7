...
// 2**15
#define EMIT_BUF_LEN 5000
#define LISTEN_BUF_LEN 40000

#define EMIT_CHIRP_PIN 11
...
// d13
const ml_pin_settings emit_trigger_pin = {PORT_GRP_A, 22, PF_A, PP_EVEN, INPUT_STANDARD, DRIVE_OFF};
// d5
const ml_pin_settings emit_ready_pin = {PORT_GRP_A, 15, PF_A, PP_ODD, OUTPUT_PULL_UP, DRIVE_ON};
...
void dac_init(void)
{
  ...
  DMAC_channel_intenset(DAC_DMAC_CHANNEL, DMAC_0_IRQn, DMAC_CHINTENSET_TCMPL, 0);
...
#define ADC0_DMAC_CHANNEL DMAC_CH1
#define ADC0_DMAC_PRILVL PRILVL0

uint16_t adc0_samples[LISTEN_BUF_LEN];
...
void adc0_init(void)
{
  ...
  DMAC_descriptor_init(
      ...
      (uint32_t)adc0_samples[0] + sizeof(uint16_t) * LISTEN_BUF_LEN,
...
#define ADC1_DMAC_CHANNEL DMAC_CH2
#define ADC1_DMAC_PRILVL PRILVL0

uint16_t adc1_samples[LISTEN_BUF_LEN];
...
void adc1_init(void)
{
  ...
  DMAC_descriptor_init(
      ...
      (uint32_t)adc1_samples[0] + sizeof(uint16_t) * LISTEN_BUF_LEN,
...
void setup()
{
  ...
  // EIC->CONFIG
  EIC->INTENSET.reg = (1 << EIC_INTENSET_EXTINT(6));
  NVIC_EnableIRQ(EIC_6_IRQn);
  peripheral_port_init(&emit_trigger_pin);
  eic_enable();

  peripheral_port_init(&emit_ready_pin);
  logical_set(&emit_ready_pin);
...
bool adc0_done, adc1_done, send_data = false;
uint16_t listen_length = LISTEN_BUF_LEN;
...
void loop()
{
  ...
  if (Serial.available() >= 1)
  {
    ...
    case ECHO_SERIAL_CMD::CHIRP_DATA:
    {
      ...
      ML_DMAC_CHANNEL_DISABLE(DAC_DMAC_CHANNEL);
      DMAC_descriptor_init(
          chirp_out_dmac_descriptor_settings,
          chirp_len,
          (uint32_t)&chirp_out_buffer[0] + chirp_len * sizeof(uint16_t),
          (uint32_t)&DAC->DATA[0].reg,
          (uint32_t)&base_descriptor[DAC_DMAC_CHANNEL],
          &base_descriptor[DAC_DMAC_CHANNEL]);
      ML_DMAC_CHANNEL_ENABLE(DAC_DMAC_CHANNEL);
    ...
    case ECHO_SERIAL_CMD::EMIT_LISTEN:
    {
      uint16_t len = Serial.read();
      len |= Serial.read() << 8;
      if (len > LISTEN_BUF_LEN)
      {
        Serial.write(ECHO_SERIAL_CMD::ERROR);
        Serial.flush();
        DOTSTAR_SET_RED();
      ...
      ML_DMAC_CHANNEL_DISABLE(ADC0_DMAC_CHANNEL);
      ML_DMAC_CHANNEL_DISABLE(ADC1_DMAC_CHANNEL);

      DMAC_descriptor_init(
          adc1_dmac_descriptor_settings,
          listen_length,
          (uint32_t)&ADC1->RESULT.reg,
          (uint32_t)adc1_samples[0] + (sizeof(uint16_t) * listen_length),
          (uint32_t)&base_descriptor[ADC1_DMAC_CHANNEL],
          &base_descriptor[ADC1_DMAC_CHANNEL]);
      DMAC_descriptor_init(
          adc1_dmac_descriptor_settings,
          listen_length,
          (uint32_t)&ADC0->RESULT.reg,
          (uint32_t)adc0_samples[0] + (sizeof(uint16_t) * listen_length),
          (uint32_t)&base_descriptor[ADC0_DMAC_CHANNEL],
          &base_descriptor[ADC0_DMAC_CHANNEL]);
  ...
  if (send_data)
  {
    if (adc1_done && adc0_done)
    {

      for (size_t i = 0; i < listen_length; i += 2)
      {
        Serial.write(adc0_samples[i] & 0xff);
        Serial.write(adc0_samples[i] >> 8 & 0xff);
        Serial.write(adc0_samples[i + 1] & 0xff);
        Serial.write(adc0_samples[i + 1] >> 8 & 0xff);
        if (i % 64 == 0)
        {
          Serial.flush();
        }
      ...
      for (size_t i = 0; i < listen_length; i += 2)
      {
        Serial.write(adc1_samples[i] & 0xff);
        Serial.write(adc1_samples[i] >> 8 & 0xff);
        Serial.write(adc1_samples[i + 1] & 0xff);
        Serial.write(adc1_samples[i + 1] >> 8 & 0xff);
        if (i % 64 == 0)
        {
          Serial.flush();
        }
...
void DMAC_0_Handler(void)
{
  ...
  if (DMAC->Channel[DAC_DMAC_CHANNEL].CHINTFLAG.bit.TCMPL)
  {
    ...
    ML_DMAC_CHANNEL_CLR_TCMPL_INTFLAG(DAC_DMAC_CHANNEL);
  }
}
...
void DMAC_1_Handler(void)
{
  ...
  {
    adc0_done = true;
    ML_DMAC_CHANNEL_CLR_TCMPL_INTFLAG(ADC0_DMAC_CHANNEL);
  }
}
...
void DMAC_2_Handler(void)
{
  ...
  {
    adc1_done = true;
    ML_DMAC_CHANNEL_CLR_TCMPL_INTFLAG(ADC1_DMAC_CHANNEL);
  }
}

void EIC_6_Handler(void)
{
  // logical_unset(&emit_ready_pin);

  // ML_DMAC_CHANNEL_CLR_TCMPL_INTFLAG(DAC_DMAC_CHANNEL);
  ML_DMAC_CHANNEL_RESUME(DAC_DMAC_CHANNEL);
  while (DMAC->BUSYCH.bit.BUSYCH0 == 0)
  {
  }
  logical_unset(&emit_ready_pin);

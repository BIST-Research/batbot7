...
void setup()
{
  ...
  digitalWrite(emit_chirp_pin,HIGH);

  pinMode(itsy_emitting_chirp,INPUT_DISABLE);
...
void loop()
{
  ...
  if (Serial.available())
  {
    ...
    case LISTENER_SERIAL_CMD::START_LISTEN:
    {
      ...
      while(!abdma1.interrupted() && !abdma2.interrupted()){
      ...
      // while (true)
      // {
      //   if(digitalReadFast(itsy_emitting_chirp) == LOW){

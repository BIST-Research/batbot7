...
def plot_and_fft(new_data:np.uint16):
    ...
    plt.plot(x_vals,new_data,'ro-',linewidth=.1,markersize=1.25)
    plt.xlabel("Time (s)")
...
def split_raw_to_LR(raw_data:np.uint16, channel_len:np.uint16, left_first = True) ->tuple[np.uint16,np.uint16]:
...
CHAN_BURST = 1000
bb_ears = bb_listener.EchoListener(Serial('/dev/tty.usbmodem136132801'),channel_burst_len=CHAN_BURST)
